...
==== Konfiguration und schnelle Erstellung von PostgreSQL-Hochverfügbarkeit

In der BlueXP -Workload-Fabrik für Datenbanken werden zwei neue Funktionen unterstützt:

* PostgreSQL HA-Konfiguration (High Availability, Hochverfügbarkeit): Sie können jetzt eine HA-Konfiguration für PostgreSQL Server implementieren.
* Schnelle Erstellung: Sie können diese schnelle Bereitstellungsoption verwenden, um einen PostgreSQL-Server mit HA-Konfiguration und eingebetteten Best Practices zu erstellen.
